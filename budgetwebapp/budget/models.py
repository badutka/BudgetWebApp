...
#             update_account(self.destination,
#                            self.amount - previous_amount)  # add the difference of amounts +(105 - 100) = +5, or +105 if +100 already rolled back
...
#             update_account(self.origin, -(
#                         self.amount - previous_amount))  # subtract the difference of amounts -(105 - 100) = -5, or -105 if -100 already rolled back
...
class Transaction(BaseModel):
    ...
    #
    #         # ROLLBACK
    #         if previous.transaction_type == 'INCOMING':
    #             update_account(previous.destination, -previous.amount)
    #         elif previous.transaction_type == 'OUTGOING':
    #             update_account(previous.origin, previous.amount)
    #         elif previous.transaction_type == 'INNER':
    #             update_account(previous.origin, previous.amount)
    #             update_account(previous.destination, -previous.amount)
    #
    #         # origin / destination could have been updated, so refresh
    ...
    #         update_account(self.origin, -self.amount)
    #         # update_account(self.origin, -self.amount)
    #     elif self.transaction_type == 'INNER':
    #         update_account(self.origin, -self.amount)
    #         update_account(self.destination, self.amount)
    #
    #     super().save(*args, **kwargs)
